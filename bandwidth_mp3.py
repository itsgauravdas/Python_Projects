...
import time 
import threading 
import subprocess # to use the comman in command prompt (unix)
import tkinter 
...
def monitor(limit, unit):
    ...
    threading.Timer(60.0, monitor, arg).start() # at 60 secs delay call the monitor function and pas  the arg as arguments
    
def main():
    if len(sys.atgv) > 3 or len(sys.argv) < 3:
        print(
