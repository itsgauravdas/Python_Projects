...
def vocabulary_quiz():
    print("AI Language Learning Assistance: Vocabulary Quiz")
    score=0
    ...
    for word in quiz_items:
        ...
        if user_answer.strip().lower() == vocabulary[word].lower():
            ...
            print(f"Wrong. The correct ans is : {vocabulary[word]}")
    print(f"Quiz completed! your score: {score}/len(vocabulary)")


   
def grammar_exercise_quiz():
    print("AI Language Assistant: Grammer Exercise")
    score=0
...
def interactive_conversation_practice():
    ...
    while True:
        ...
        user_input = input("Chatbot " + conversations['question'] + " ")
...
def detect_pronunciation_errors():
    ...
    try:
        user_sentance = recognizer.recognize_google(audio)
        ...
        correct_sentance = "I Love learning Language" 
        if user_sentance.strip().lower() == correct_sentance.lower():
